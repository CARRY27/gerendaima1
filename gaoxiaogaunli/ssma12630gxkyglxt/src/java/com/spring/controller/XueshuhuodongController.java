 ...
 */
@Controller
public class XueshuhuodongController extends BaseController {

    @Autowired
    private XueshuhuodongMapper dao;

    @Autowired
    private XueshuhuodongService service;
     ...
     *
     */
    @RequestMapping("/xueshuhuodong_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        List<Xueshuhuodong> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "xueshuhuodong_list"; // 使用视图文件：WebRoot\xueshuhuodong_list.jsp
    }
    ...
    public String getWhere() {
        ...
        if (!Request.get("huodongshumu_start").equals("")) {
            where += " AND huodongshumu >='" + Request.get("huodongshumu_start") + "' ";
        }
        if (!Request.get("huodongshumu_end").equals("")) {
            where += " AND huodongshumu <= '" + Request.get("huodongshumu_end") + "' ";
        }
     ...
     * 开发人员列表
     */
    @RequestMapping("/xueshuhuodong_list_kaifarenyuan")
    public String listkaifarenyuan() {
        ...
        List<Xueshuhuodong> list = service.selectPage(example, page, 12);

        request.setAttribute("list", list);
        assign("orderby", order);
        assign("sort", sort);
        assign("where", where);
        return "xueshuhuodong_list_kaifarenyuan";
    }

    /**
     * 添加人列表
     */
    @RequestMapping("/xueshuhuodong_list_tianjiaren")
    public String listtianjiaren() {
        ...
        assign("sort", sort);
        assign("where", where);
        return "xueshuhuodong_list_tianjiaren";
    }

    @RequestMapping("/xueshuhuodong_add")
    public String add() {
        if (!checkLogin()) {
        ...
        return "xueshuhuodong_add";
    }

    @RequestMapping("/xueshuhuodong_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Xueshuhuodong mmm = service.find(id);
        request.setAttribute("mmm", mmm);
        request.setAttribute("updtself", 0);
        return "xueshuhuodong_updt";
    }
     ...
     * @return
     */
    @RequestMapping("/xueshuhuodonginsert")
    public String insert() {
        String tmp = "";
        Xueshuhuodong post = new Xueshuhuodong(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        post.setHuodongshumu(Request.getInt("huodongshumu"));
     ...
     * @return
     */
    @RequestMapping("/xueshuhuodongupdate")
    public String update() {
        // 创建实体类
        Xueshuhuodong post = new Xueshuhuodong();
        // 将前台表单数据填充到实体类
        ...
        if (!Request.get("huodongshumu").equals("")) post.setHuodongshumu(Request.getInt("huodongshumu"));
     ...
     *  后台详情
     */
    @RequestMapping("/xueshuhuodong_detail")
    public String detail() {
        if (!checkLogin()) {
            return showError("尚未登录", "./");
        }
        int id = Request.getInt("id");
        Xueshuhuodong map = service.find(id); // 根据前台url 参数中的id获取行数据
        request.setAttribute("map", map); // 把数据写到前台
        return "xueshuhuodong_detail"; // 详情页面：WebRoot\xueshuhuodong_detail.jsp
    }

    /**
     *  删除
     */
    @RequestMapping("/xueshuhuodong_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("xueshuhuodong").find(id);
