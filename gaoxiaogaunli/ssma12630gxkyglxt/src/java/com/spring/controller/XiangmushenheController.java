 ...
 */
@Controller
public class XiangmushenheController extends BaseController {

    @Autowired
    private XiangmushenheMapper dao;

    @Autowired
    private XiangmushenheService service;
     ...
     *
     */
    @RequestMapping("/xiangmushenhe_list")
    public String list() {
        // 检测是否有登录，没登录则跳转到登录页面
        if (!checkLogin()) {
        ...
        List<Xiangmushenhe> list = service.selectPage(example, page, 12); // 获取当前页的行数
        // 将列表写给界面使用
        assign("list", list);
        assign("orderby", order); // 把当前排序结果写进前台
        assign("sort", sort); // 把当前排序结果写进前台
        assign("where", where); // 把当前条件写给前台
        return "xiangmushenhe_list"; // 使用视图文件：WebRoot\xiangmushenhe_list.jsp
    }
    ...
    public String getWhere() {
        ...
        if (!Request.get("shenheren").equals("")) {
            where += " AND shenheren LIKE '%" + Request.get("shenheren") + "%' ";
        }
        return where;
    }
     ...
     * 开发人员列表
     */
    @RequestMapping("/xiangmushenhe_list_kaifarenyuan")
    public String listkaifarenyuan() {
        ...
        List<Xiangmushenhe> list = service.selectPage(example, page, 12);

        request.setAttribute("list", list);
        assign("orderby", order);
        assign("sort", sort);
        assign("where", where);
        return "xiangmushenhe_list_kaifarenyuan";
    }

    /**
     * 审核人列表
     */
    @RequestMapping("/xiangmushenhe_list_shenheren")
    public String listshenheren() {
        ...
        assign("sort", sort);
        assign("where", where);
        return "xiangmushenhe_list_shenheren";
    }

    @RequestMapping("/xiangmushenhe_add")
    public String add() {
        if (!checkLogin()) {
        ...
        return "xiangmushenhe_add";
    }

    @RequestMapping("/xiangmushenhe_updt")
    public String updt() {
        int id = Request.getInt("id");
        // 获取行数据，并赋值给前台jsp页面
        Xiangmushenhe mmm = service.find(id);
        request.setAttribute("mmm", mmm);
        request.setAttribute("updtself", 0);
        return "xiangmushenhe_updt";
    }
     ...
     * @return
     */
    @RequestMapping("/xiangmushenheinsert")
    public String insert() {
        String tmp = "";
        Xiangmushenhe post = new Xiangmushenhe(); // 创建实体类
        // 设置前台提交上来的数据到实体类中
        ...
        DB.execute("update xiangmu set zhuangtai='" + request.getParameter("shenhe") + "' where xiangmubianhao='" + request.getParameter("xiangmubianhao") + "'");
     ...
     * @return
     */
    @RequestMapping("/xiangmushenheupdate")
    public String update() {
        // 创建实体类
        Xiangmushenhe post = new Xiangmushenhe();
        // 将前台表单数据填充到实体类
     ...
     *  后台详情
     */
    @RequestMapping("/xiangmushenhe_detail")
    public String detail() {
        if (!checkLogin()) {
            return showError("尚未登录", "./");
        }
        int id = Request.getInt("id");
        Xiangmushenhe map = service.find(id); // 根据前台url 参数中的id获取行数据
        request.setAttribute("map", map); // 把数据写到前台
        return "xiangmushenhe_detail"; // 详情页面：WebRoot\xiangmushenhe_detail.jsp
    }

    /**
     *  删除
     */
    @RequestMapping("/xiangmushenhe_delete")
    public String delete() {
        if (!checkLogin()) {
            return showError("尚未登录");
        }
        int id = Request.getInt("id"); // 根据id 删除某行数据
        Map map = Query.make("xiangmushenhe").find(id);
