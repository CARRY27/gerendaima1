...
<div style="padding: 10px" class="admin-content">
    <div class="panel panel-default">
        <div class="panel-heading">
            <span class="module-name"> 通告 </span>
            <span>列表</span>
        </div>
                    ...
                    <tbody>
                        <c:set var="i" value="0" /><c:forEach items="${list}" var="map"
                            ...
                            <tr id="${map.id}" pid="">
                                ...
                                <td>${map.faburen}</td>
                                <td align="center">
                                    <a href="tonggao_detail.do?id=${map.id}">详情</a>
                                    <c:choose>
                                        <c:when test="${sessionScope.cx == '管理员' }">
                                            <a href="tonggao_updt.do?id=${map.id}">编辑</a>
                                        </c:when></c:choose
                                    >
                                    <c:choose>
                                        <c:when test="${sessionScope.cx == '管理员' }">
                                            <a href="tonggao_delete.do?id=${map.id}" onclick="return confirm('确定要删除？')">删除</a>
                                        </c:when></c:choose
                                    >
                                    <!--qiatnalijne-->
                                </td>
